...
int main()
{
    ...
    forin(i,n){
        forisn(j,i+1,n)
          {
            if(v[i]/r==v[j]/r && i+1!=j)
            {
                if(suma(1,0,n-1,i+1,j-1)/r==(suma(1,0,n-1,j+1,n-1)+suma(1,0,n-1,0,i-1))/r)
                {
                    cout<<"Y\n";
                    return 0;
                }
    ...
    /*forin(i,n/2)
    {
        if(v[i]/r==v1[i]/r || v[i]/r==(v1[i]/r)+v1[i+1]/r ){
                if(as(i,v)==as(i,v1) || as(i,v)==as(i+1,v1)-v1[i] ){
                cout<<"Y\n";
                return 0;}
                }
    }*/
